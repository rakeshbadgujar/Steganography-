...
public class Analyze {
	...
	public Analyze(int[][][] threeD,int rows,int cols,int alpha) {
            System.out.println("The alpha ain analyse() are: "+alpha);
            System.out.println("The rows ain analyse() are: "+rows/8);
            System.out.println("The rows ain analyse() are: "+cols/8);
            int i,j;
		plane0= new byte[rows][cols][4];
		for(i=0;i<rows;i++) {
			for(j=0;j<cols;j++) {
				//nothing in the alpha array
//                            System.out.println("In Analyse first for loop Start and i="+i+" j ="+j);
				plane0[i][j][0]=0;
				//R
...
//                                System.out.println("In Analyse first for loop end ");
			}
		}
	...
	BLOCK_SIZE=8;
	    ...
	    blocks_in_a_row=cols/BLOCK_SIZE;
            System.out.println("Blocks in a ror "+blocks_in_a_row);
			...
			for(x=0;x<blocks_in_a_row;x++) {
				for(i=(y*BLOCK_SIZE),a=0;i<(y*BLOCK_SIZE+8);i++,a++) {
					for(j=(x*BLOCK_SIZE),b=0;j<(x*8 + 8);j++,b++) {
						//r
//                                             System.out.println("In Analyse for second loop");
